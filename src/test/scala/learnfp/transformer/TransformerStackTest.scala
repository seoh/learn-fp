    ...
    "work" in {
      {
        for {
          x <- 10.pure[App]
          _ <- StateT.lift[String, OuterT, Unit](WriterT.tell(List("een")))
          _ <- StateT.putT[String, OuterT]("one")
          y <- 20.pure[App]
          _ <- StateT.lift[String, OuterT, Unit](WriterT.tell(List("twee")))
          _ <- StateT.putT[String, OuterT]("two")
          z <- 30.pure[App]
          _ <- StateT.lift[String, OuterT, Unit](WriterT.tell(List("drie")))
